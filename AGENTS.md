...
- 验收单入口: `pnpm run verify` = `typecheck` + `test:suite` + `test:smoke`（现行口径与 skip 规则见 `docs/testing/runtime-refact-test-migration.md`）.
...
- Real Host: `pnpm run test:real-host` — exact 0.1.5-rc.2 真实 Host 组合；与受控 smoke 分开报告，不得混称.
