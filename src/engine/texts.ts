 ...
 * (absorbs the catalog discipline semantics + send_message override clause).
 * Appended to every actor-task dispatch — no token needed, the dispatch lease
 * binds the claim. */
export const SUBMISSION_CONSTRAINT = `\n\n[提交要求]\n结果只通过 node_claim 交付：outcome（completed | failed）+ 唯一 handoff（trim 后 1..8000 字符，END 节点也必须交付），无需 token，绑定由派发自动完成。handoff 是 Judge、Manager、后继与最终结果共用的唯一交接文本：实际完成/失败内容、产物位置与核验依据、剩余问题和后续约束；旧 summary/handoffContext 参数不再接受。\nnode_claim 必须是本轮对话的最后一个动作：调用后本 turn 其余输出全部无效，不要再输出文字或调用任何工具；仅输出文字不视为提交，会导致当前 Node BLOCK。不要向父会话 send_message 汇报结果——本工作流的交付与推进只认 node_claim（此条优先于宿主附加的通用 send_message 指引）。\n额度不足、缺少条件或临时无法继续：node_block（reason 说明问题、证据、缺失信息与需要 Manager 决定的事项），不伪报 failed。\n收到 Judge REJECT 后：认可则按意见修正并重新 node_claim；意见与既有 criteria 或可验证事实冲突、超出范围或缺权限时，node_block 说明分歧、证据与需要 Manager 决定的事项，不为迎合判定伪报 failed。`
