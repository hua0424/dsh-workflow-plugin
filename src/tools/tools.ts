...
/**
 * Authorize a control tool call through THE given instance's host and resolve its
 * workspace key + caller id. The same `host` then serves the rest of the call, so
 * authorize and the following mutation/inspection can never straddle two instances.
 */
async function controlWorkspace(host: ToolHost, agent: unknown, toolName: string): Promise<{ workspaceKey: string; caller: string } | { workspaceKey: null; reason: string }> {
  const auth = await host.authorize(agent, toolName)
...
/**
 * #94：工具定义在插件实例装配时以闭包绑定该实例的 ToolHost（沿用命令工厂思路），
 * 不再有模块级可变引用——同名工具跨实例互不串用，一次调用中的 authorize 与
 * 后续 mutation/inspection 必然落在同一个 Host 上。
 */
export function makeWorkflowTools(host: ToolHost): ToolDefinition[] {
  return [
    defineTool({
      ...
      async execute(args, exec) {
        const auth = await controlWorkspace(host, exec.agent, 'workflow_status')
        ...
        if (!historyRequested) return fmtResult(await host.status(auth.workspaceKey, auth.caller))
        ...
        return fmtResult(await host.status(auth.workspaceKey, auth.caller, {
    ...
    defineTool({
      ...
      output: stringOut,
      async execute(args, exec) {
        const auth = await controlWorkspace(host, exec.agent, 'node_block')
        if (auth.workspaceKey === null) return `拒绝：${auth.reason}`
        ...
        const outcome = await host.block(auth.workspaceKey, args.nodeToken, args.reason.trim(), claimCallerOf(exec))
        if (outcome.ok) exec.concludeTurn()
        return fmtResult(outcome)
      },
    }),
    ...
    defineTool({
      ...
      output: stringOut,
      async execute(args, exec) {
        const auth = await controlWorkspace(host, exec.agent, 'node_resume')
        if (auth.workspaceKey === null) return `拒绝：${auth.reason}`
        ...
        return fmtResult(await host.resume(auth.workspaceKey, args.nodeToken, args.resolutionContext.trim(), auth.caller, args.target ?? 'auto'))
      },
    }),
    ...
    defineTool({
      ...
      output: stringOut,
      async execute(args, exec) {
        const auth = await controlWorkspace(host, exec.agent, 'node_run_program')
        if (auth.workspaceKey === null) return `拒绝：${auth.reason}`
        return fmtResult(await host.runProgram(auth.workspaceKey, args.nodeToken, args.parameters, auth.caller))
      },
    }),
    ...
    defineTool({
      ...
      output: stringOut,
      async execute(args, exec) {
        const auth = await controlWorkspace(host, exec.agent, 'node_resolve_program')
        if (auth.workspaceKey === null) return `拒绝：${auth.reason}`
        const reasonError = lengthError('reason', args.reason, 1, LIMITS.blockReasonMax, true)
        if (reasonError !== undefined) return `拒绝：${reasonError}`
        return fmtResult(await host.resolveProgram(auth.workspaceKey, args.nodeToken, args.result, args.reason.trim(), auth.caller))
      },
    }),
    ...
    defineTool({
      ...
      output: stringOut,
      async execute(args, exec) {
        const auth = await controlWorkspace(host, exec.agent, 'judge_respawn')
        if (auth.workspaceKey === null) return `拒绝：${auth.reason}`
        ...
        return fmtResult(await host.respawnJudge(auth.workspaceKey, args.nodeToken, args.reason?.trim(), auth.caller))
      },
    }),
    ...
    defineTool({
      ...
      output: stringOut,
      async execute(args, exec) {
        const auth = await controlWorkspace(host, exec.agent, 'judge_claim')
        if (auth.workspaceKey === null) return `拒绝：${auth.reason}`
        const reasonError = lengthError('reason', args.reason, LIMITS.reasonMin, LIMITS.reasonMax, true)
        if (reasonError !== undefined) return `拒绝：${reasonError}`
        const outcome = await host.judgeClaim(auth.workspaceKey, args.nodeToken, args.result, args.reason.trim(), claimCallerOf(exec))
        if (outcome.ok) exec.concludeTurn()
        return fmtResult(outcome)
      },
    }),
    ...
    defineTool({
      ...
      output: stringOut,
      async execute(args, exec) {
        const auth = await controlWorkspace(host, exec.agent, 'workflow_inspect_git')
        if (auth.workspaceKey === null) return `拒绝：${auth.reason}`
        return fmtResult(await host.inspectGit(auth.workspaceKey, args.operation))
      },
    }),
    ...
    defineTool({
      ...
      output: stringOut,
      async execute(args, exec) {
        const auth = await controlWorkspace(host, exec.agent, 'workflow_inspect_github')
        if (auth.workspaceKey === null) return `拒绝：${auth.reason}`
        return fmtResult(await host.inspectGithub(auth.workspaceKey, args.operation, args.milestoneNumber))
      },
    }),
  ]
}
