...
test('rapid merge 拒绝旧 stale-review 出口，不调度审查或提前结束', async () => {
  const h = harness()
  try {
    await implement(h)
    const before = await h.step('review', 'approved')
    const invalid = await h.engine.handleClaim('ws', { result: 'stale-review', handoff: '受控提交旧结果，不模拟真实修订漂移' }, h.caller(before.execution.dispatch!))
    assert.equal(invalid.ok, false)
    const unchanged = await h.row()
    assert.equal(unchanged.execution.nodeId, 'merge')
    assert.equal(unchanged.execution.executionId, before.execution.executionId)
    assert.equal(unchanged.execution.nodeToken, before.execution.nodeToken)
    assert.deepEqual(unchanged.execution.dispatch, before.execution.dispatch)
