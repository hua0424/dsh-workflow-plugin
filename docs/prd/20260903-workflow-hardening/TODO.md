...
| [A4](a4-cold-resume-compaction-investigation.md) | Cold-resume Compaction 调查 | 🔍 代码层调研完成，待实现与运行时验证 | 11 | — | 见 §3 与 [a4-code-findings.md](a4-code-findings.md) |
...
- 待办：按方案 A 实现 + 隔离 harness 补运行时证据（A4 PRD §4/AC1/AC3/AC4/AC6），数据齐备后按 AC9 回写设计文档。

## 4. 待办清单

- [ ] A4：按方案 A 实现 cold materialize 压缩 + 隔离 harness 运行时验证（代码层调研已完成，见 [a4-code-findings.md](a4-code-findings.md)）
