...
- 本目录状态：A2（配置强化）已按 v1 兼容语义落地并部署；A4 方案 A 已实现并合并 main（待部署与运行时验证，见 [a4-code-findings.md](a4-code-findings.md)）；A3 已实现（分支 `a3-trace-observability`，fmt=2 trace 事件全覆盖，见 PRD §13 实现记录）；A1/A5 仍为需求与方案文档，尚未实现
