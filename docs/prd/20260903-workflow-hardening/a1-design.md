...
| D2 | Judge `revision` 字段（A3 遗留 AC4） | **不加**。REJECT 轮换 nodeToken 并退役 Judge（清 `judgeSessionId`），旧 Judge 重复提交被 token/judgeSession 双重拒绝；trace 去重继续用 token 前缀。A3 遗留项就此关闭 | 本设计 §6.4 |
| D3 | MODEL provider/modelId 长度上限（A3 遗留） | catalog 校验 + `handleSetRoleModel` 增加 LIMITS：provider ≤ 64、modelId ≤ 128 字符（trim 后） | 本设计 §9 |
...
新文件 `src/plugin/turnbind.ts`，纯函数、可独立单测：

```ts
/** 本次工具调用所在 turn 内全部 user/message 的 message id；无法推导时 undefined（fail-closed）。 */
...
1. 从尾向头找 `tool/call` 且 `data.callId === callId` 的事件，取其 `turn`（找不到 → undefined：本 call 尚未落 log，理论不可达，仍 fail-closed）；
2. 继续倒序走，记录沿途所有 `user/message` 事件的 `data.id`，直到遇到 `turn/start` 且 `turn` 相同的事件为止（含边界内、tool/call 之前进入 turn 的全部 user message）；
3. 返回 id 集合。扫描过程中若先遇到更小 turn 的 `turn/start`/`turn/end` 而未命中目标 turn 起点 → undefined（log 异常，fail-closed）。

接线：`src/tools/tools.ts` 的 `node_claim` / `node_block` 在 authorize 通过后，用 `exec.agent.session.events` + `exec.callId` 计算集合（在 enqueue 之前快照——turn 内 id 集合只增不改，快照安全），随 caller 传给 host → engine。

## 3. ActorDispatchLease（R2）

```ts
/** 一次已成功发出的 actor-task dispatch 的 claim 准入凭证（A1 R2，内存态）。 */
interface ActorDispatchLease {
  workspaceKey: string
  nodeToken: string            // 该 dispatch 对应的 topFrame nodeToken
  executorSessionId: string    // manager 节点 = managerSessionId
  dispatchMessageId: string    // followup/startContinuable/steer 的 user message id
  consumed: boolean
}
```

存放：`WorkflowEngine` 内存 `Map<workspaceKey, ActorDispatchLease>`（串行引擎，每 workspace 至多一个活跃 lease）。**不持久化**：host 重启后 running run 一律 BLOCK（现有 restart-reconcile），恢复必经 resume → 重新 dispatch → 新 lease，符合 R2"不尝试从不完整的内存 lease 猜测恢复"。

### 3.1 发布（dispatch 成功后）

`dispatchCurrent()` 三条路径各自在 **send 成功返回后**发布/覆盖 lease（send 抛错则不发布——"dispatch 失败不得产生可 claim lease"）：

| 路径 | dispatchMessageId 来源 | executorSessionId |
|---|---|---|
| manager 节点 steer | `steerManager` 新返回值（§4） | `run.managerSessionId` |
| 已有 role actor followup | `sendRoleActor` 返回的 messageId | `run.roleActors[roleKey]` |
| 新建 role actor | `ensureRoleActor` 返回的 messageId | `started.childId` |
...
- `handleClaim` 准入通过并完成最终 re-read 后置 `consumed = true`；同一 lease 的第二个 claim 拒绝（AC4）。
- `node_block`：当 caller 是当前 node 的精确 executor 时，走与 claim 相同的 lease 绑定检查并消费（见 §5.2）。

### 3.3 失效（防御性双保险）

显式清理点：`advance()` 任何节点离开（edge/END/pop）、`handleBlock`、`blockOnJudgeFault`、`handleJudgeTurnEnded`、`handleRunProgram` ERROR BLOCK、`handleRestartReconcile`（clear 全部）、`handleReset`、REJECT correction（token 轮换 + 重派，新 dispatch 覆盖）。

即使清理遗漏，admission 仍校验 `lease.nodeToken === topFrame(run).nodeToken && run.status === 'running' && lease.executorSessionId === callerSessionId`，token 轮换后旧 lease 自然失配——清理是收敛加速，不是正确性依赖。
...
5. **lease 准入**：
   - lease 不存在 / `lease.nodeToken !== topFrame(run).nodeToken` / `lease.executorSessionId !== caller.sessionId` / `lease.consumed` / `lease.dispatchMessageId ∉ caller.turnUserMessageIds` → 拒绝，文案：`当前调用无法绑定到一个已 dispatch 的 Node`（显式覆盖 PRD 问题 1：State 已 advance、Node 未 dispatch 时无匹配 lease，必然落此分支，不改 State、不 spawn Judge——AC1）；
6. `run.pendingClaim !== undefined` / inFlight 检查（现状保留）；
7. 最终 re-read（entered 状态复核）中 token 校验改用 `lease.nodeToken`（此时与 topFrame 必相等，防御性保留）；
8. 通过后置 `consumed = true`，再走现有 judge spawn 流程。

executor 精确性由 lease 隐含（lease.executorSessionId 即 dispatch 目标），原 `executorSessionOf` 显式检查保留为断言级冗余，不承担正确性。
...
claim 被接受后进入 judgment phase（`pendingClaim` + `judgeSessionId` 已持久化）：lease 已 consumed，同 turn/新 turn 的重复 claim 在第 5 步被拒——与现状 `a judgment is already pending` 双保险，拒绝理由更早更准。
...
| 任意 | NEED_CONTEXT | 现状不变：BLOCK + 保留 judge/pendingClaim/boundary（AC9） |

关键变化：`advance()` 的 verdict 不再取 Judge result，而是由 **Actor claim outcome 映射**（completed→PASS、failed→FAIL）——Judge 只确认，不改写结果（PRD §2 目标 2/3）。`advance()` 函数签名与内部行为不变。

### 6.2 correction 流（按序）

1. `logJudge(result=REJECT, reason)`（A3 §10：validate → trace → persist）；
2. `retireJudge` + `delete run.judgeSessionId` + `delete run.pendingClaim`（R6.1/6.2）；
3. `frame.nodeToken = newNodeToken()`；workflowId/nodeId 不变（R6.3）；
4. `nodeBoundary` **保留**（R8）：role 路径重派时 `dispatchCurrent` 的 `isSameNodeResume` 判定为真 → 不 compact、boundary 不重置；manager 路径 `establishManagerBoundary` 见 `dispatchedAt !== 0` 提前返回——两条路径均天然满足，无需新逻辑；
5. 写 trace 新事件 **`CORRECT`**（§7.2）；
6. 构造 correction 消息（R7 格式，§6.3）；
7. 派发决策复用现有 workerSettled/book 机制：actor turn 仍活跃 → `persistDeferred(workspaceKey, run, version, correctionBody)`；已 settle → `dispatchNow(..., correctionBody)`（与 PASS/FAIL 后的下一节点派发完全同构，F13 语义不变）；
8. 重派成功 → 新 lease 自动发布（§3.1）；旧 lease 因 token 轮换失配。

### 6.3 correction 消息格式（R7）

`dispatchCurrent` 的 transient 包装头参数化：`'handoff' | 'correction'`（默认 handoff，现有 `[handoff]` 文案不变）。correction 的完整消息：
...
R8"下一 Judge 能看到先前 claim 与 rejection"由现有 projection 天然满足：correction followup 是 actor session 的 `user/message`（含 judge rejection 原文），落在 retained boundary 窗口内，`projectNodeLocal` 会投影给下一 Judge。

### 6.4 不加 revision 字段（D2）

REJECT 后：token 轮换 + judgeSessionId 清空 + Judge 授权撤销（`retireJudge`）。旧 Judge 若再次 `judge_claim`：token stale（`frame.nodeToken !== nodeToken`）或 judge session 不匹配，双重拒绝；trace 侧 JUDGE 行带各自 token 前缀，无去重歧义。
...
- `CONTEXT.md`（Judge 结果语义词条）、`docs/design/configurable-agent-workflow-graph.md`（§5.2 工具协议、§2.2 Judge 职责）、`README.MD`（trace JUDGE 取值、协议示例）；
...
- `LIMITS` 新增 `providerMax = 64`、`modelIdMax = 128`（trim 后长度）。
- 校验点：catalog `roles[*].modelRoute` / `judge.modelRoute`（`catalog/schema.ts` 加 refinement）；`handleSetRoleModel` 入口（现有非空检查旁加长度检查，超限拒绝并提示上限）。
- `logModel` 无需改（输入已受界）；A3 遗留项关闭。
...
| AC2 | dispatch 成功后（makeHarness 真实走 dispatchNow）不传 nodeToken 的精确 executor claim 被接受，JUDGE 流程正常 | 现有 harness；caller 换 `ClaimCaller`，turnUserMessageIds 含 dispatch id |
...
| AC7/AC8 | REJECT：无 ROUTE/JUDGE-FAIL 行、nodeId/workflowId 不变、token 轮换、CORRECT 行、correction 消息含 [judge rejection]/[previous claim]/[instruction] 三段、boundary 保留（dispatchedAt 不重置） | 消息文本断言 + trace 断言 |
| AC9 | NEED_CONTEXT 现状回归 | 现有用例改 enum 值 |
...
| AC12 | 全量 unit + e2e；turnbind.test.ts 纯函数用例（callId 定位、mid-turn steer、log 截断/异常 fail-closed） | node:test |

既有 169 用例改造面：`NodeClaim` 去字段、`handleClaim` 签名、judge enum 改名、checker id 改名、SUBMISSION_CONSTRAINT 断言、e2e smoke 的 judge 脚本与断言。

## 11. 实现顺序（映射 PRD §10）

1. `turnbind.ts` + `steerManager` messageId（seam 先行，独立测试）；
2. lease + admission gate（handleClaim/node_block）→ AC1–AC4/AC11；
3. `node_claim` schema 去 nodeToken + SUBMISSION_CONSTRAINT 文案；
4. judge v2 enum + 真值表 → AC5/AC6/AC10；
5. REJECT correction 流 + CORRECT 事件 + transient 头参数化 → AC7/AC8；
6. v2 原地升级 + checker 更名 + 配置/示例/文档迁移 + MODEL 上限（D3）；
7. 全量验证（build + unit + e2e），PRD §13 收口，README/CONTEXT/设计文档同步。
