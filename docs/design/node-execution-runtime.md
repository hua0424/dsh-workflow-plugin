...
- 状态：T1–T9 实现与 A01–A30 自动化验收已完成、终审并合入 `main`（`refact` 分支为历史）。验收证据见 [`node-execution-runtime-acceptance.md`](../testing/node-execution-runtime-acceptance.md)；现行收口入口见 [`runtime-refact-test-migration.md`](../testing/runtime-refact-test-migration.md)。部署需用户显式授权。
- 运行基线：DSH `0.1.5-rc.2`（本设计冻结时的基线为 `0.1.2-rc.1`，2026-09-11 升级，见 issue #37 / PR #38）；源码与索引位置、升级实测记录见根目录 `AGENTS.md`。
...
- Child 调用节点用必填 `onReturn` 替代 onPass：键集合必须与被调用流程的 `returns` **精确相等**（缺映射/多余映射在 catalog 校验期拒绝），值是**本层** Target。调用节点没有自己的结果集，它的裁决名就是被调用流程的返回名。
- 子流程内某个节点走到 `{ return: <本流程返回名> }` 即结束该层：引擎 pop 一层 frame，按父调用节点 `onReturn[返回名]` 取得上一层 Target，再决定是继续同层后继还是继续向上一层返回。两层及以上连续返回在**同一次事务**里逐层应用本层映射，允许每层重命名；最底层结果名不会未经映射穿透到 Root。
- 每层 caller 记录被调用流程的终局来源 `executionId` 与 handoff 原文（`child.result`），并保留自己的裁决名（`returned` = 被调用流程实际返回的名字）；来源链可沿 `terminalExecutionId` 逐层追溯。handoff 只有一份原文，逐层原样传递、不重摘要。
